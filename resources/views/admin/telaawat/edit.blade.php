<x-admin.layouts.app title="تعديل تلاوة">
    ...
    <h1 class="text-3xl font-bold text-white mb-8">تعديل: {{ $telaawat->name }}</h1>

    <form method="POST" action="{{ route('admin.telaawat.update', $telaawat) }}" enctype="multipart/form-data" class="max-w-2xl space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="sheikh_id" required
                ...
                @foreach ($sheikhs as $sheikh)
                    <option value="{{ $sheikh->id }}" {{ old('sheikh_id', $telaawat->sheikh_id) == $sheikh->id ? 'selected' : '' }}>{{ $sheikh->name }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $telaawat->name) }}" required
        ...
        <div>
            ...
            <textarea name="description" rows="3"
                class="w-full bg-gray-800 text-white border border-gray-700 rounded-lg px-4 py-3 text-base placeholder-gray-500 focus:outline-none focus:border-blue-500 transition">{{ old('description', $telaawat->description) }}</textarea>
            @error('description') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <x-label>الملف الصوتي</x-label>
            @if ($telaawat->audio_url)
                <p class="text-sm text-gray-500 mb-2">الملف الحالي: {{ $telaawat->audio_url }}</p>
            @endif
